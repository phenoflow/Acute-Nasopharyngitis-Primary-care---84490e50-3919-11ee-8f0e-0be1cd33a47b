...
(async()=>{
  ...
  await fs.writeFile('acute-nasopharyngitis-primary-care-potential-cases.csv', 'patient-id,dob,codes,last-encounter\n');
  for(let patient in patients) {
    try {
      const row = patient+','+ages[patient]+',\"'+Array.from(patients[patient]).join(',')+'\",'+lastEncounters[patient].slice(0,-1)+'\n';
      console.log("row:" + row);
      await fs.appendFile('acute-nasopharyngitis-primary-care-potential-cases.csv', row);
    } catch(error) {
      console.log(error);
    }
  }
